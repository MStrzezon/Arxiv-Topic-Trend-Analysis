...
class CustomTop2Vec(Model):
    def __init__(self, model_config = None, number_of_keywords=30):
    ...
    def train_with_metrics_calculation(self, dataset) -> pd.DataFrame:
        ...
        model = Top2Vec(data, embedding_model=self.model_config['embedding_model'], ngram_vocab=self.model_config['ngram_vocab'])
        ...
        model_output = get_topvec_model_output(data, model.doc_top, topics_words, topics_vectors, document_vectors)
        model_metrics = calculate_metrics(model_output, dataset)
        results_df = pd.DataFrame([model_metrics])
        results_df.insert(0, 'model name', f'Top2Vec - embedding_model:{self.model_config["embedding_model"]}, ngram_vocab:{self.model_config["ngram_vocab"]}')
        return results_df
